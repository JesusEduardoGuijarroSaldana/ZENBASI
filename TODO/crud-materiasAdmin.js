    ...
    $.ajax({
        ...
        success: function(respuesta){
            ...
            if(miObjetoJSON.estado==1){
                ...
                if(nivelAcademico == 1){
                    tabla.row.add([nombreMateria, "Kinder", botones]).draw().node().id="renglon_"+miObjetoJSON.id;
                }else if(nivelAcademico == 2){
                    tabla.row.add([nombreMateria, "Primaria", botones]).draw().node().id="renglon_"+miObjetoJSON.id;
                }else
                    tabla.row.add([nombreMateria, "Secundaria", botones]).draw().node().id="renglon_"+miObjetoJSON.id;
            ...
            if(miObjetoJSON.estado==1){
                ...
                miObjetoJSON.materias.forEach(Nombre => {
                    ...
                    if(Nombre.NivelAcademico == 1){
                        tabla.row.add([Nombre.Nombre, "Kinder", botones]).draw().node().id="renglon_"+Nombre.idMateria;
                    }else if(Nombre.NivelAcademico == 2){
                        tabla.row.add([Nombre.Nombre, "Primaria", botones]).draw().node().id="renglon_"+Nombre.idMateria;
                    }else
                        tabla.row.add([Nombre.Nombre, "Secundaria", botones]).draw().node().id="renglon_"+Nombre.idMateria;                    
                });
            }
        }
    })
